...
protected:
  ...
  bool LoadPacketsFromFile(const std::string& filename);
  ...
  bool LoadPacketsFromOtherFiles(const std::string& filename);
  ...
  bool DeleteIncompleteTargetFiles(void);
    ...
    // Signals
  virtual void SigFilename(std::string filename) {}
  virtual void SigProgress(int progress) {}
  virtual void SigHeaders(ParHeaders& headers) {}
  virtual void SigDone(std::string filename, int available, int total) {}
...
protected:
  ...
  std::unordered_set<std::string>           par2Set;     // set of par2 files
